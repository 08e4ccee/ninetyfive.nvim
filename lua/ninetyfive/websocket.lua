...
local function send_file_content()
    ...
    gitignore_cache.is_ignored(bufnr, function(ignored)
        if ignored then
...
function Websocket.setup_autocommands()
    ...
    vim.api.nvim_create_autocmd({ "BufReadPost" }, {
        ...
        callback = function(args)
            ...
            -- Update gitignore status cache for this buffer
            gitignore_cache.update_status(bufnr)
    ...
    -- Update gitignore cache when switching buffers
    ...
    --         end
    --
    --         if vim.b[bufnr].ninetyfive_gitignored == nil then
    --             gitignore_cache.update_status(bufnr)
    --         end
    --     end,
    -- })
...
function Websocket.shutdown()
    ...
    local info = debug.getinfo(2) -- Get all info about caller
    print("Caller file: " .. (info.short_src or "unknown"))
    print("Caller line: " .. (info.currentline or 0))
    print("Caller function: " .. (info.name or "anonymous"))
